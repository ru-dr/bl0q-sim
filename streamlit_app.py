...
with st.sidebar.expander("👥 Team & Operating Costs"):
    ...
    if enable_salaries:
        ...
        monthly_team_cost = 0
        team_grow_enabled = False
        team_grow_percent = 0.0
        team_grow_period = "Monthly"
...
gross_profit_per_customer = gross_profit / num_customers if num_customers > 0 else 0
ltv = gross_profit_per_customer * avg_customer_lifetime_months
...
st.header("🔮 AI Forecast: Revenue, Profit, Customers & More")

# Simulate 24 months of historical data for forecasting
...
if team_grow_enabled:
    if team_grow_period == "Monthly":
        period = 1
    elif team_grow_period == "Quarterly":
        period = 3
    elif team_grow_period == "Yearly":
        period = 12
    else:
        period = 1
...
forecasted_profit = forecast_linear(hist_profit)
forecasted_customers = forecast_linear(hist_customers)
forecasted_projects = forecast_linear(hist_projects)
forecasted_cac = forecast_linear(hist_cac)
forecasted_ebitda = forecast_linear(hist_ebitda)
forecasted_gross_profit = forecast_linear(hist_gross_profit)
forecasted_team_cost = forecast_linear(hist_team_cost)
forecasted_ltv = forecast_linear(hist_ltv)
forecasted_margin = forecast_linear(hist_margin)
...
fig, axs = plt.subplots(4, 3, figsize=(18, 16), facecolor="#0a0a0a")
fig.subplots_adjust(hspace=0.4)
metrics = [
    ("Revenue", forecasted_revenue, hist_revenue),
    ("Net Profit", forecasted_profit, hist_profit),
    ("Customers", forecasted_customers, hist_customers),
    ("Projects", forecasted_projects, hist_projects),
    ("CAC", forecasted_cac, hist_cac),
    ("EBITDA", forecasted_ebitda, hist_ebitda),
    ("Gross Profit", forecasted_gross_profit, hist_gross_profit),
    ("Team Cost", forecasted_team_cost, hist_team_cost),
    ("LTV", forecasted_ltv, hist_ltv),
    ("Margin (%)", forecasted_margin, hist_margin),
    ("COGS", forecasted_cogs, hist_cogs),
]
...
ai_metrics = [
    ("Revenue", forecasted_revenue[-1], growth_rate(forecasted_revenue)),
    ("Net Profit", forecasted_profit[-1], growth_rate(forecasted_profit)),
    ("Customers", forecasted_customers[-1], growth_rate(forecasted_customers)),
    ("Projects", forecasted_projects[-1], growth_rate(forecasted_projects)),
    ("CAC", forecasted_cac[-1], growth_rate(forecasted_cac)),
    ("EBITDA", forecasted_ebitda[-1], growth_rate(forecasted_ebitda)),
    ("Gross Profit", forecasted_gross_profit[-1], growth_rate(forecasted_gross_profit)),
    ("Team Cost", forecasted_team_cost[-1], growth_rate(forecasted_team_cost)),
    ("LTV", forecasted_ltv[-1], growth_rate(forecasted_ltv)),
    ("Margin (%)", forecasted_margin[-1], growth_rate(forecasted_margin)),
    ("COGS", forecasted_cogs[-1], growth_rate(forecasted_cogs)),
]

# Show as a grid of metrics
cols = st.columns(5)
